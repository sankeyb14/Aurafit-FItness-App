...
export function sanitizeParsedPlan(planData) {
  if (!planData || !Array.isArray(planData.weeks)) return planData;

  const sanitizedWeeks = planData.weeks.map((week, wIdx) => ({
    ...week,
    days: (week.days || []).map((day, dIdx) => ({
      ...day,
      exercises: (day.exercises || []).map((ex, eIdx) => {
        const selectedKey = ex.userSelectedVariant || 'primary';

        const primaryRaw = ex.primary || (ex.name ? ex : null) || ex;
        const altARaw = ex.altA || ex.alternativeA || ex.easierVariant;
        const altBRaw = ex.altB || ex.alternativeB || ex.harderVariant;

        const normalizeVar = (v, fallbackName) => {
          if (!v) return null;
          if (typeof v === 'string') {
            return {
              name: v, sets: 3, reps: '10', restSeconds: 90, muscleGroup: 'Full Body',
              why: 'Target exercise for strength development.',
              formTips: ['Maintain good posture', 'Control movement tempo', 'Breathe out on exertion'],
              commonMistakes: ['Rushing reps', 'Improper joint alignment'],
              injuryPrevention: 'Brace core and move under control.'
            };
          }
          return {
            name: v.name || v.exerciseName || v.title || v.exercise || fallbackName,
            sets: v.sets || v.set_count || 3,
            reps: String(v.reps || v.rep_count || '10'),
            restSeconds: v.restSeconds || v.rest_seconds || 90,
            muscleGroup: v.muscleGroup || v.target_muscle || 'Full Body',
            why: v.why || v.description || 'Target exercise for strength development.',
            formTips: Array.isArray(v.formTips) && v.formTips.length > 0 ? v.formTips : ['Maintain good posture', 'Control movement tempo'],
            commonMistakes: Array.isArray(v.commonMistakes) && v.commonMistakes.length > 0 ? v.commonMistakes : ['Rushing reps', 'Improper joint alignment'],
            injuryPrevention: v.injuryPrevention || 'Brace core and protect joints.'
          };
        };

        return {
          exerciseId: ex.exerciseId || `ex_w${wIdx+1}_d${day.dayNum || dIdx+1}_${eIdx}`,
          userSelectedVariant: selectedKey,
          primary: normalizeVar(primaryRaw, 'Target Exercise'),
          altA: normalizeVar(altARaw, 'Incline Push-up (Easier Variant)'),
          altB: normalizeVar(altBRaw, 'Weighted Progression (Harder Variant)')
        };
      })
    }))
  }));

  return { ...planData, weeks: sanitizedWeeks };
}
...
export function generateLocalFallbackPlan(userProfile) {
  ...
  return sanitizeParsedPlan(rawPlan);
}
...
`;
    ...
    return sanitizeParsedPlan(fullPlan);
  } catch (error) {
