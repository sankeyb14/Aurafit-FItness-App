...
import { Flame, CheckSquare, Square, Clock, Plus, Moon, Battery, ChevronRight, Info, Utensils, Trash2, Calendar, Sparkles, AlertCircle, Play, ShieldAlert } from 'lucide-react';

export function DashboardPage() {
  const { profile } = useUser();
  const { plan, updateExerciseVariant } = usePlan();
  const { todayLog, dailyLogs, streakInfo, logWorkoutCompletion, deleteMealLog, addMealLog } = useLogs();

  const [selectedExerciseItem, setSelectedExerciseItem] = useState(null);
  const [isMealModalOpen, setIsMealModalOpen] = useState(false);
  const [adaptationModal, setAdaptationModal] = useState(null); // { type: 'reps_missed' | 'low_recovery', exName: '...' }
  ...
  const handleSaveActualReps = (exItem) => {
    const planned = Number(exItem.primary?.reps?.split('-')[0]) || 10;
    const actual = Number(actualRepsInput) || 8;

    if (actual < planned) {
      setAdaptationModal({
        type: 'reps_missed',
        exName: exItem.primary?.name || 'Exercise',
        planned,
        actual
      });
    } else {
  ...
  return (
    <div className="min-h-screen pb-24 pt-4 px-4 sm:px-6 max-w-md mx-auto sm:max-w-lg md:max-w-xl space-y-6 animate-fadeIn bg-[#F3F4F8]">
      ...
      {/* TOP HEADER - Matches Reference Blueprint */}
      ...
      <div className="bg-[#FF5E3A] text-white p-6 rounded-3xl shadow-lg relative overflow-hidden flex justify-between items-center">
        <div className="space-y-3 z-10 max-w-[68%]">
          <span className="inline-block bg-white/20 backdrop-blur-md px-3.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
            Week {currentWeekNum} • {todayWorkoutDay?.dayTitle || 'Shoulder & Upper Body'}
          </span>
      ...
      {/* TODAY'S EXERCISES (WITH PRIMARY + ALTERNATIVE VARIANTS) */}
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          {(todayWorkoutDay?.exercises || []).map((exItem, idx) => {
            const variantKey = exItem.userSelectedVariant || 'primary';
            const variant = exItem[variantKey] || exItem.primary || exItem;
              ...
              <div key={idx} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[9px] font-black uppercase text-[#7C3AED] bg-purple-50 px-2.5 py-0.5 rounded-full">
                      {variant.muscleGroup || 'Chest'} • {variantKey.toUpperCase()}
                    </span>
                    <h4 className="text-sm font-black text-gray-900 mt-1">{variant.name}</h4>
                    <p className="text-xs text-gray-500 font-semibold">{variant.sets} Sets × {variant.reps} Reps • {variant.restSeconds || 90}s rest</p>
                  </div>

                  <button
                    onClick={() => setSelectedExerciseItem(exItem)}
                    className="text-xs font-bold text-[#7C3AED] bg-purple-50 px-3 py-1.5 rounded-xl hover:bg-purple-100 transition"
                  >
                    Form & Alts →
                  </button>
                </div>
                ...
                <div className="pt-2 border-t border-gray-100 flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      defaultValue={variant.reps?.split('-')[0] || 10}
      ...
      {/* ADAPTATION MODAL (WHEN REPS MISSED OR RECOVERY LOW) */}
      {adaptationModal && (
